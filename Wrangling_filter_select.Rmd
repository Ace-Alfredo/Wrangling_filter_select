...
```
There was 1,248 movies made in 1995.


2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?
```{r}
long_highrated_Comedy <-movies %>%
  
filter(Comedy ==1, !mpaa=="", rating> 8,length > 120) %>%
  select(title, year, rating, length, mpaa)
long_highrated_Comedy
```

There are only 4 comedy movies that have a high rating above 8 and that is longer then 120 minutes.
...
The following Harry Potter movies include in this data are Harry Potter and the chamber of secrets, Harry Potter and the prisoner of Azkaban, then finally, Harry Potter and the sorcerer Stone. The highest rating harry potter movie is according to this data set is  Harry Potter, and the prisoner of Azkaban. According to imdb.com, Harry Potter, and the sorcerer Stone has the most votes with 46,741.   
...
The highest movie budget that is a comedy or a animation is Wild Wild West. Their budget consist of 170,000,000 and had 19078 votes. 
